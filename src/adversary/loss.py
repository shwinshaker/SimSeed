...
__all__ = ['kd_loss', 'bce_loss', 'trades_loss', 'llr_loss', 'mart_loss', 'fat_loss', 'gairat_loss']


def kd_loss(outputs, target_scores, T=2.):
    ...
    log_scores_norm = F.log_softmax(outputs / T, dim=1)
    ...
    KD_loss_unnorm = -(target_scores * log_scores_norm)
...
def gairat_loss(net, X, y, weights=None,
    ...
    inputs_ad, stats = attack(net, ctr, X, y, weight=None,
                              adversary='pgd',
                              eps=config.eps,
