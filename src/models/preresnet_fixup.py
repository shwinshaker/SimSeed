...
__all__ = ['preresnet_fixup']
# ['FixupResNet', 'fixup_resnet20', 'fixup_resnet32', 'fixup_resnet44', 'fixup_resnet56', 'fixup_resnet110', 'fixup_resnet1202']
...
class FixupBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bias2c = nn.Parameter(torch.zeros(1))
        self.downsample = downsample

    def forward(self, x):
        identity = x

        out = self.relu(x + self.bias1b)
        out = self.conv1(out + self.bias1a)

        out = self.relu(out + self.bias2c)
...
class FixupResNet(nn.Module):
    ...
    def __init__(self, depth, block=FixupBasicBlock, num_classes=10, n_channel=3, gain=1.0):
        super(FixupResNet, self).__init__()
        assert (depth - 2) % 6 == 0, 'When use basicblock, depth should be 6n+2, e.g. 20, 32, 44, 56, 110, 1202'
        layers = [(depth - 2) // 6] * 3 # [3, 3, 3] - resnet-20
        self.num_layers = sum(layers)
        self.inplanes = 16
        self.conv1 = conv3x3(n_channel, 16)
        ...
        for m in self.modules():
            if isinstance(m, FixupBasicBlock):
                nn.init.normal_(m.conv1.weight, mean=0, std=gain * np.sqrt(2 / (m.conv1.weight.shape[0] * np.prod(m.conv1.weight.shape[2:]))) * self.num_layers ** (-0.5))
...
def preresnet_fixup(**kwargs):
    return FixupResNet(**kwargs)


def fixup_preresnet20(**kwargs):
