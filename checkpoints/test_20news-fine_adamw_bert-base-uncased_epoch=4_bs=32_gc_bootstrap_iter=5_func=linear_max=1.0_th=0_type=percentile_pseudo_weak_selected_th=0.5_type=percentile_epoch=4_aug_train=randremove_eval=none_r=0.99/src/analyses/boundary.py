...
def is_parallel(path):
    import yaml
    with open('%s/config.yaml' % path) as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    return ',' in str(config['gpu_id'])

def get_net(path, num_classes, n_channel, device, model='resnet', bn=True, depth=20, width=16, feature=None, state='model.pt'):
    ...
    if 'vgg' in model:
        ...
        net = models.__dict__[model](num_classes=num_classes, n_channel=n_channel).to(device)
