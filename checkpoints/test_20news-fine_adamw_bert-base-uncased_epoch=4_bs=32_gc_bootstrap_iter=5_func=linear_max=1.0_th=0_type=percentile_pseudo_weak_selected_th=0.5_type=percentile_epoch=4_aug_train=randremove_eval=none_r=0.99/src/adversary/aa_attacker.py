...
class AAAttacker:
    ...
    def __init__(self, net, eps=8.,
        ...
        if data_dir is not None:
            ...
            self.__set_dataset(dataset, data_dir)
    ...
    def __set_dataset(self, dataset, data_dir):
        # auto attack assume a [0,1] input
        # therefore need to rebuild the dataset here

        if dataset == 'mnist':
            dataloader = datasets.MNIST
            to_idx = lambda x: x.item() # targets in mnist are torch tensors
        elif dataset == 'cifar10':
            dataloader = datasets.CIFAR10
            to_idx = lambda x: x # targets in cifar are integers
        ...
        self.testset = dataloader(root=data_dir,
                                  train=False,
                                  transform=transform_chain,
                                  download=False)
        self.testset.targets = [to_idx(t) for t in self.testset.targets]

        self.trainset = dataloader(root=data_dir,
                                   train=True,
                                   transform=transform_chain,
                                   download=False)
        self.trainset.targets = [to_idx(t) for t in self.trainset.targets]
